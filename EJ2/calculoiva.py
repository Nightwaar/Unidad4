...
class calculariva():
    ...
    def __init__(self):
        ...
        ttk.Button(self.__ventana, text='Salir', style="Red.TButton", command=self.__ventana.destroy).place(x=300, y=325)
        style1 = ttk.Style()
        style1.configure("Green.TButton", background="green")
        ttk.Button(self.__ventana, text='Calcular', style="Green.TButton", command=self.calcular).place(x=80, y=325)
        ...
        ttk.Radiobutton(seleccionar,text="IVA 10.5%",value=1,variable=self.iva,command=self.calcular()).place(x=130,y=220)
